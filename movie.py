	...
	for day in days:
		...
		for hour in range(24):
			...
			for drifter in range(passed+1, min(day+1, 32)):  # drifters that are alive on day
				...
				if depth:
					drif = ax.scatter( dd["lonp"][412:,-idrift], dd["latp"][412:,-idrift], c=abs(dd["zp"][412:,-idrift]), cmap=cmo.cm.deep, vmin=0, vmax=500, marker="o", edgecolors="black", s=12, linewidths=0.5, transform=ccrs.PlateCarree() )
